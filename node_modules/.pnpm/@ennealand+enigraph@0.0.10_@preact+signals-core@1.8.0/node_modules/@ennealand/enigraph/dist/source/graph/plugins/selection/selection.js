...
export const withSelection = (props) => {
    const localize = props.localize ?? ((x, y) => [x, y]);
    const area = useSignal(null);
    const progress = useSignal(new Set());
    const values = useSignal(new Set());
    const postponedClickedNodeId = useSignal(null);
    const clearSelection = useCallback(() => {
        if (values.value.size)
            values.value = new Set();
    }, [values]);
    const startSelection = (e, options) => {
        ...
        const newProgress = options?.clear ? new Set() : new Set(values.value);
        if (clickedNode) {
            const processSingleClick = (values) => {
                if (!options?.deselection && !(props.inversion && !options?.selection && values.has(clickedNode.id))) {
                    values.add(clickedNode.id);
                }
                else
                    values.delete(clickedNode.id);
            };
        ...
        batch(() => {
            ...
            if (options?.clear && !dontClear)
                values.value = new Set();
        });
        document.addEventListener('mouseup', stopSelection, { once: true });
    };
    const updateSelection = (e, options) => {
        if (!area.value)
            return;
    ...
    return {
        ...
        AreaSelection: useCallback(() => area.value && _jsx(AreaSelection, { ...area.value }), [area]),
        selection: useComputed(() => area.value ? progress.value : values.value),
        isSelecting: useComputed(() => !!area.value && (area.value.x1 !== area.value.x2 || area.value.y1 !== area.value.y2)),
        clearSelection,
        startSelection,
        updateSelection,
    };
